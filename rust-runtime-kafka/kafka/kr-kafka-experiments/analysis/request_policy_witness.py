...
from request_policy_review import policy_contract
...
def decimal_fields(value):
    """Presentation witnesses preserve every integer exactly in JSON readers."""
    if type(value) is int:
        return str(value)
    if isinstance(value, list):
        return [decimal_fields(v) for v in value]
    if isinstance(value, dict):
        return {k: decimal_fields(v) for k, v in value.items()}
    return value
...
def first_divergence(raw, wire):
    ...
    for field in ('external_history', 'environment'):
        ...
        result[field] = {'identical_prefix_entries': index,
                         'sealed': a[index] if index < len(a) else None,
                         'broker-ready': b[index] if index < len(b) else None}
    return result
...
def main():
    ...
    paths = {'sealed': args.sealed, 'broker-ready': args.broker_ready}
    reports = {mode: load_report(path) for mode, path in paths.items()}
    require(policy_contract(reports['sealed'], 'sealed') == policy_contract(reports['broker-ready'], 'broker-ready'),
    ...
    output = {'schema': 'kr-request-policy-witness/v1',
              'integer_encoding': 'All integer values are canonical decimal strings; booleans remain booleans. This includes counters, indices, signed fields and byte-array elements, as well as full-width IDs and clocks.',
              ...
              'first_divergence': first_divergence(reports['sealed'], reports['broker-ready']),
              'analysis_tools_sha256': {name: digest(Path(__file__).parent / name) for name in
                  ('request_policy_witness.py', 'request_policy_review.py', 'classic_request_witness.py',
                   'classic_artifacts.py', 'classic_audit.py', 'classic_comparison.py', 'classic_visualization.py')},
              'runs': runs}
    args.out.parent.mkdir(parents=True, exist_ok=True)
    write_json(args.out, decimal_fields(output))
